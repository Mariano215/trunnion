...
| `audit.finding` | 10 · 11 | One thing a workload asserts about the material it was shown: `class` (one of `secret`, `dependency.provenance`, `authz.boundary`, and nothing else loads), `path`, `line`, the one-sentence `claim` in the asserter's own words, `asserted_by` naming the provider and model, `evidence` as the ids of the events the claim rests on, and `status`, which is `asserted` and not `confirmed`. The kind exists so a finding is a leaf and `trunnion ledger prove` can cover it: a finding kept beside the log could only ever carry proofs of its evidence while the claim itself stayed text anyone could edit afterwards. It is deliberately not a `sensor.verdict`, which means a check with a fix and negative controls ran; a model assertion has none of those, and reusing that kind would have the scorer credit primitive 10 for prose. Nothing scores from `audit.finding`: a claim is not a control, and crediting a level for making one would pay for volume. A finding carries no id of its own: the event id is its identity, and the per-run id it did carry collided the first time an audit ran over more than one file, which is what a second identity for something the ledger already identifies is worth. `trunnion report` numbers them in log order for the document it writes. Added in slice 25. |
